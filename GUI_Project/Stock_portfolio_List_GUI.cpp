...
//XXXXX---------------------------------------------------------------------------------------------------------------------------------XXXXXXX
double List::buy_stock(string sym1, double num, double min_amt)
{
	double return_value_GUI = 0;
	...
	if (num*min_amt <= getCashBalance() && min_amt >= curr_value)			//if capable to buy the share
	{
		...
		while (searchsym != nullptr && searchsymflag == 0)
		{
			if (sym1 == searchsym->getSym())
			{
				...
				return_value_GUI = curr_value;			//sent the current value to print it in the dialog box
				Update_CashBalance_into_txt(CashBalance + (curr_value*num));
			}
		...
		setCashBalance_new(CashBalance);
...
//XXXXX---------------------------------------------------------------------------------------------------------------------------------XXXXXXX
double List::sell_stock(string sym, double num, double amount_offered)
{
	...
	else                                                //symbol present in portfolio
	{
		ListNode *tempptr = firstptr;
		...
		if (current_number_shares > num)				//if you have more shares than you want to sell
		{
			...
			if (sort_choice == 1)
				...
				this->selection_sort();*/
			this->bubble_sort();
			//setCashBalance_new(getCashBalance() + (num * return_share_per_price_from_LinkedList(sym)));
		...
		else if (current_number_shares == num)			//if exactly the amount you want to sell
		{
			...
			stock_transaction_history(0, sym, num, tempptr->PPS, tempptr->gettotalvalue());		//add the transaction
			//setCashBalance_new(getCashBalance() + (num * return_share_per_price_from_LinkedList(sym)));
			Update_CashBalance_into_txt(CashBalance);		//update it otherwise it will be lost
			update_transaction_on_account_stockinfo(0, num*return_share_per_price_from_LinkedList(sym)); //update in bank account, neccessary to be after the cashbalance has been updated
			tempptr->setData(0);
			/*int sort_choice;
			cout << "There are two sorting techniques available.\n";
...
{
	ofstream copy;
	copy.open("Current_portfolio_GUI.txt");										//replace the contents
	ListNode *tempptr = firstptr;
	while (tempptr != nullptr)
	{
...
void List::updateList()						//every time the system makes a list, a new file is generated
{
	...
	read.open("Current_portfolio_GUI.txt");
	ListNode *tempptr = firstptr;
...
void List::stock_transaction_history(int check, string sym, double num, double pricepstock, double total_value)
{
	...
	write_trans.open("stock_transaction_history_GUI.txt", ios::app);			//append and add at the end
...
void List::totalPortfolioValue()
{
	...
	ofstream write_TPV;
	write_TPV.open("Total_portfolio_Value_GUI.txt", ios::app);
	write_TPV << setw(22) << totalPV + getCashBalance() << "	"  /*returncurrenttime() */<< endl;
	write_TPV.close();
	//--------forMatlab
	write_TPV.open("Total_portfolio_Value_in_Sec_for_Matlab_GUI.txt", ios::app);
	write_TPV << setw(22) << totalPV + getCashBalance() << "	" /*returncurrenttime() */ << endl;
	write_TPV.close();
}
//XX-------------------------------------------------------------------------------------------------------------------------------------------XX
...
void List::update_transaction_on_account_stockinfo(int ch, double money)
{
	...
	{
		write_trans << "BUYYShare" << "	" << money << "		" << getCashBalance() << "		" /*<< returncurrenttime() */<< endl;
		write_trans.close();
	}
	else
	{
...
void List::read_stock_transaction_history()
{
	ifstream read_bank;
	string item;
	read_bank.open("stock_transaction_history_GUI.txt");
...
CString List::print_out_history_stock()
{
	ifstream read_bank;

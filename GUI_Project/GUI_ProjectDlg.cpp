...
CGUIProjectDlg::CGUIProjectDlg(CWnd* pParent /*=nullptr*/)
	...
	, m_instructions(_T("Instructions"))
	, m_amount(_T("Enter"))
	, m_result(_T("Result"))
	, amount_update(_T(""))
	, m_sym(_T("Enter"))
...
BEGIN_MESSAGE_MAP(CGUIProjectDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON_BS, &CGUIProjectDlg::OnBnClickedButtonBs)
	ON_BN_CLICKED(IDC_BUTTON_SS, &CGUIProjectDlg::OnBnClickedButtonSs)
	ON_BN_CLICKED(IDC_BUTTON_stock_tran_print, &CGUIProjectDlg::OnBnClickedButtonstocktranprint)
END_MESSAGE_MAP()
...
void CGUIProjectDlg::OnBnClickedButton1()					//DEPOSIT MONEY
{
	...
	BA.update_transaction_on_account(1, d);
	CString convert;
...
void CGUIProjectDlg::OnBnClickedButton2()					//WITHDRAW MONEY
{

	UpdateData(true);						//recieve value
	m_amount = m_amount;					//uopdate value into the variable, therefore true
	double d = _tstof(m_amount);			//convert it to double
	if (BA.getCashBalance() >= d)
	{
		BA.setCashBalance_new(BA.getCashBalance() - d);			//update the new value of cashbalance
		BA.update_transaction_on_account(0, d);
		CString convert;
		convert.Format(_T("%g"), BA.getCashBalance());								//copnvert the new value to str
	...
	{
		m_result = _T("Not enough balance!.");
		UpdateData(false);
	}
}
void CGUIProjectDlg::OnBnClickedButton3()
{
	BA.Update_CashBalance_into_txt(BA.getCashBalance());
	BA.setCashBalance_new(BA.getCashBalance());
...
void CGUIProjectDlg::OnBnClickedButtonBs()				//buy shares
{
	UpdateData(true);
	m_sym = m_sym;
	m_num = m_num;
	...
	CStringA m_sym1(m_sym);
	string send(m_sym1);
	double d1 = _tstof(m_num);
	double d2= _tstof(m_sp);
	double return_value = SA.buy_stock(send,d1,d2);
	if (return_value >0)
	{
		CString convert1;
		convert1.Format(_T("%g"), return_value);
		m_result = m_num + _T(" shares for the company " )+ m_sym + _T(" bought at the price $") + convert1+_T(" each.");
		double d = BA.getCashBalance();
		double sp = SA.return_stockprice(send);
		BA.setCashBalance_new(d-(sp*d1));
	}
	...
	{
		m_result = _T("Price per share is more than what offered.");
	}
	else if (return_value == -1)
	{
		m_result = _T("Symbol not found.");
	}
	UpdateData(false);
}


void CGUIProjectDlg::OnBnClickedButtonSs()					//sell shares
{
	UpdateData(true);
	m_sym = m_sym;
	m_num = m_num;
	...
	double d1 = _tstof(m_num);
	double d2 = _tstof(m_sp);
	double return_value = SA.sell_stock(send, d1, d2);
	if (return_value > 0)
	{
		CString convert1;
		convert1.Format(_T("%g"), return_value);
		m_result = m_num + _T(" shares for the company ") + m_sym + _T(" sold at the price $") + convert1 + _T(" each.");
		double d = BA.getCashBalance();
		double sp = SA.return_stockprice(send);
		BA.setCashBalance_new(d+(sp*d1));
	}
	...
	{
		m_result = _T("Symbol for the stock not found in the portfolio.");
	}
	else if (return_value == -4)
	{
		m_result = _T("You dont have that many shares!");
	}
	else if (return_value == -1)
	{
		m_result = _T("Price per stock is lower than the amount the user is willing to sell for");
	}
	UpdateData(false);
...
void CGUIProjectDlg::OnBnClickedButtonstocktranprint()
{
	CString display = SA.print_out_history_stock();
	m_result = display;
	UpdateData(false);
}
